...
class Framework {
	...
	*/
	static public function setup($application_document_root, $server_document_root) {
		...
		View::$path = static::$pathToProject . 'application/views/';

		// determine dynamic uri path
		static::$uriPathDynamic = static::parseDynamicUri($server_document_root, $application_document_root);

		// determine the relative path to the public directory
		static::$uriPathStatic = static::parseStaticUri($server_document_root, $application_document_root);
		...
		if (!file_exists($server_document_root . static::$uriPathStatic)) {
			static::$uriPathStatic = '/';
			static::$uriPathDynamic = '/';
		}
	}
	...
	*/
	static public function dispatch($request, $server_query_string, $server_request_method) {
		...
		$request += Router::parse($request['_uri'] . '?' . $server_query_string, $request['_method'], $request, static::$uriPathDynamic);
		...
		// make request
		$response = static::control($request);
	...
	*/
	static public function control($request) {    
		...
		try {
			...
			if (is_dir(static::$pathToProject . 'application/views/' . $request['_controller'])) {
				$view = $request['_controller'] . '/' . $request['_action'];
				$request['_controller'] = 'app';
				$controller = new \AppController();
				$controller->view($view);
			} elseif (is_file(static::$pathToProject . 'application/views/' . $request['_controller'] . '/' . $request['_action'] . '.' . $request['_format'])) {
	...
	*/
	static public function url($url) {
		if ($url == null || $url == '/') {
			return static::$uriPathDynamic;
		}

		if (substr($url, 0, 1) != "#") {
			if (substr($url, 0, 1) == '/') {
				$url = static::$uriPathStatic . substr($url, 1, strlen($url));
			} elseif (! preg_match('/^[a-z]+:/', $url, $matchs)) {
				$url = static::$uriPathDynamic . $url;
			}
		}
		return $url;
	}
