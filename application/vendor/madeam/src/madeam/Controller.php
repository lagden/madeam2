...
class Controller {
	...
	*/
	public function process($request) {
		...
		// set ajax if no set
		isset($request['_ajax']) ?: $request['_ajax'] = 0;
		...
		if ($this->_output === null) {
			...
			try {
				...
				// serialize output
				if (isset(static::$formats[$request['_format']]) && method_exists(static::$formats[$request['_format']][0], static::$formats[$request['_format']][1])) {
					$this->_output = call_user_func(static::$formats[$request['_format']], $this->_data);
				} else {
					throw $e;
				}
			}
	...
	*/
	public function callback($name, $request) {
		if($request['_ajax']===1)return false;
		if (!isset($this->_callbacks[$name]))return false;
