...
class HasMany extends AbstractRelationship
{
	 ...
	 * <li><b>throughClass:</b> name of a class model</li>
	 * </ul>
	 *
	 * @var array
	 */
	// Modificado
	static protected $valid_association_options = array('primary_key', 'order', 'group', 'having', 'limit', 'offset', 'through', 'throughClass', 'source');
	...
	public function __construct($options=array())
	{
		...
		if (isset($this->options['through']))
		{
			...
			// Criado / Modificado
			if (isset($this->options['throughClass']))$this->throughClass = $this->options['throughClass'];
	...
	public function load(Model $model)
	{
		...
		if (!isset($this->initialized))
		{
			if ($this->through)
			{
				...
				// Modificado
				$through_table = Table::load(classify((($this->throughClass)?$this->throughClass:$this->through), true));
